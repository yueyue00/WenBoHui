...
public class AsyncTaskForRequestVipDaHuiInfos_ByDate extends
	...
	public AsyncTaskForRequestVipDaHuiInfos_ByDate(Message m, String userid,Context context,String user) {
		this.m = m;
		this.userid = userid;
		this.user = user;
	...
	protected Integer doInBackground(Integer... params) {
		...
		try {
			...
			System.out.println(Constant.DOMAIN+"/meetings.do?method=getMeetInfoByDate&lg="+SharedData.getInt("i18n", Language.ZH)+"&vipid="+userid+"&userid="+user);
			//URL url = new URL(Constant.DOMAIN+"/meetings.do?method=getMeetInfoByDate&lg="+SharedData.getInt("i18n", Language.ZH)+"&vipid="+userid+"&userid="+user);
			URL url = new URL(Constant.DOMAIN+"/meetings.do?method=getMeetInfoByDate&lg="+SharedData.getInt("i18n", Language.ZH)+"&vipid="+userid);
			...
			if (is != null) {
				...
				if (bpojo.code.equals("200")) {// 请求成功
					...
					m.obj = infos;
					e.putString("gerenhuiyirichengbydate", Constant.encode(Constant.key,bpojo.info.toString()));
					e.apply();
					return 1;
