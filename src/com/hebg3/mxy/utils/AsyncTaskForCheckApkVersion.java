...
public class AsyncTaskForCheckApkVersion extends AsyncTask<Integer,Integer,Integer>{
	...
	protected Integer doInBackground(Integer... params) {
		...
		try {
			URL url=new URL(Constant.DOMAIN+"/appApk.do?method=appEdition&htype=2&type=1");			
			InputStream is=HttpUrlConnectionutil.getHttpUrlConnectionisGET(url,context);//传入url和请求方式
			if(is!=null){
...
//			    	Type type=new TypeToken<ArrayList<XiaoXiPojo>>(){}.getType();//设置集合type
			    	NewApkVersionPojo pojo=g.fromJson(bpojo.info,NewApkVersionPojo.class);//解析多层jsonelement数据
			    	m.obj=pojo;
			    	return 1000;
			    }else{
			    	return -1000;
