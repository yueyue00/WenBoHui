...
public class AsyncTaskForDownloadFile extends AsyncTask<Object, Object, Object> {

	Context cont;
	String applicationdownloadurl;
	String filename = "test.txt";// 下载文件的文件名
	SharedPreferences sp;

	public AsyncTaskForDownloadFile(Context cont, String applicationdownloadurl) {
		this.cont = cont;
		sp = cont.getSharedPreferences("WIC", Activity.MODE_PRIVATE);
		System.out
				.println("aaa:AsyncTaskForDownloadTask:applicationdownloadurl"
						+ applicationdownloadurl);
		this.filename = applicationdownloadurl.substring(applicationdownloadurl
				.lastIndexOf("/") + 1);
	...
	protected Object doInBackground(Object... arg0) {
		...
		// 下载文件存放路径提前设置
		File f = new File(Environment.getExternalStoragePublicDirectory("WIC")
				.getPath());
		if (!f.exists()) {
			f.mkdir();
		}
		// 如果存在apk就先删除
		File apk = new File(Environment
				.getExternalStoragePublicDirectory("WIC").getPath() + filename);
		if (apk.exists()) {
			apk.delete();
		}
		...
		request.setDestinationInExternalPublicDir("WIC", filename);
