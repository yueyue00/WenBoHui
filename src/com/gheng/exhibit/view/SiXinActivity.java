...
public class SiXinActivity extends BaseActivity implements OnClickListener,
		OnRefreshListener {
	...
	private BroadcastReceiver mBroadcastReceiverSiXin = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (action.equals(ACTION_NAME)) {
				...
				String servicename = intent.getExtras()
						.getString("servicename");
				String content = intent.getExtras().getString("content");
				if (serviceidb.equals(serviceid)) {// 刷新
	...
	protected void onNewIntent(Intent intent) {
		...
		sixinadapter = new SiXinAdapter(this, 1, "");
	...
	Handler handler = new Handler() {
		public void handleMessage(Message msg) {
			...
			if (msg.what == 0) {
				...
				BaseActivity.gotoLoginPage(SiXinActivity.this);
			} else {// 请求失败
				Toast.makeText(context, BaseActivity.getLanguageString("请求失败"),
						Toast.LENGTH_SHORT).show();
	...
	public void onClick(View arg0) {
		...
		if (but_fanhui.getId() == arg0.getId()) {
			imm.hideSoftInputFromWindow(edittext_sixin.getWindowToken(), 0);
