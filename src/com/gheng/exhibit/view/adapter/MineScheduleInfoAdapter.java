 ...
 * @author lileixing
 */
public class MineScheduleInfoAdapter extends ScheduleInfoAdapter {

	/**
	 * @param context
	 */
	public MineScheduleInfoAdapter(Context context) {
		super(context);
	}
		...
		ApiUtil.postFav(infoData.getId(), Constant.TYPE_SCHEDULE_INFO, mode, new ApiCallBack() {
			...
			public void callback(boolean success, long id, int type, int mode, Object data) {
				...
				if (success) {
					...
					if (mode == 0) {
						infoData.isfav = 0;
						MineScheduleInfoAdapter.this.data.remove(infoData);
					} else {
						infoData.isfav = 1;
					}
