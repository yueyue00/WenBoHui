...
public class SegmentedGroup extends RadioGroup {
	...
	public SegmentedGroup(Context context) {
		...
		mMarginDp = (int) getResources().getDimension(
				R.dimen.radio_button_stroke_border);
		mCornerRadius = getResources().getDimension(
				R.dimen.radio_button_conner_radius);
	...
	private void initAttrs(AttributeSet attrs) {
		TypedArray typedArray = getContext().getTheme().obtainStyledAttributes(
				attrs, R.styleable.SegmentedGroup, 0, 0);

		try {
			mMarginDp = (int) typedArray.getDimension(
					R.styleable.SegmentedGroup_border_width, getResources()
							.getDimension(R.dimen.radio_button_stroke_border));

			mCornerRadius = typedArray.getDimension(
					R.styleable.SegmentedGroup_corner_radius, getResources()
							.getDimension(R.dimen.radio_button_conner_radius));

			mTintColor = typedArray.getColor(
					R.styleable.SegmentedGroup_tint_color, getResources()
	...
	private void updateBackground(View view) {
		...
		// Set proper radius
		((GradientDrawable) checkedDrawable).setCornerRadii(mLayoutSelector
				.getChildRadii(view));
		((GradientDrawable) uncheckedDrawable).setCornerRadii(mLayoutSelector
				.getChildRadii(view));
	...
	/*
	 * This class is used to provide the proper layout based on the view. Also
	 * provides the proper radius for corners. The layout is the same for each
	 * selected left/top middle or right/bottom button. float tables for setting
	 * the radius via Gradient.setCornerRadii are used instead of multiple xml
	 * drawables.
	 */
	private class LayoutSelector {
		...
		private final float r1 = TypedValue.applyDimension(
				TypedValue.COMPLEX_UNIT_DIP, 0.1f, getResources()
						.getDisplayMetrics()); // 0.1 dp to px
		...
		public LayoutSelector(float cornerRadius) {
			children = -1; // Init this to force setChildRadii() to enter for
							// the first time.
			child = -1; // Init this to force setChildRadii() to enter for the
						// first time
			r = cornerRadius;
		...
		private void setChildRadii(int newChildren, int newChild) {
			...
			// If same values are passed, just return. No need to update
			// anything
			...
			if (children == 1) {
				...
				radii = (getOrientation() == LinearLayout.HORIZONTAL) ? rLeft
						: rTop;
			} else if (child == children - 1) { // right or bottom
				radii = (getOrientation() == LinearLayout.HORIZONTAL) ? rRight
						: rBot;
			} else { // middle
				radii = rMiddle;
			}
